...
import {
  CategoryActionType,
  CreateCategoryActionType, DeleteCategoryActionType,
...
export const CreateEditAction = async (dispatch: Dispatch<CreateCategoryActionType>, model: ICategoryEdit) => {
  const formData = new FormData();
  formData.append("image", {
    uri: model.image,
    type: "image/jpeg",
  ...
  formData.append("name", model.name);
  formData.append("description", model.description);
  const resp = await http_common.post(`/api/categories/edit/${model.id}`,
    formData, {
      headers: {
        "Content-Type": "multipart/form-data"
      }
